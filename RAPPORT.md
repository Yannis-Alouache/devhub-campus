...
- **Manipulation.** Commit `00ac500` sur `main`, en forcant `image.tag=does-not-exist-step8` dans `platform/apps/dev/annuaire.yaml`.
...
- **Manipulation.** Commit `a17bb48` sur `main`, ajoutant un job `annuaire-dev-annuaire-migration` avec `argocd.argoproj.io/hook: PreSync`.
...
- **Manipulation.** Le meme commit `a17bb48` introduit un `ConfigMap` annote `argocd.argoproj.io/sync-wave: "-1"` et un `Deployment` annote `argocd.argoproj.io/sync-wave: "0"`. Ensuite, le commit `650e851` retire la cle `LOG_LEVEL` du `ConfigMap` (`data: {}`) et force un rollout visible avec `maxUnavailable: 1` et `maxSurge: 0`.
...
- **Manipulation.** Commit `4e74c15` sur `main`, avec `prune: true` active pour `annuaire-dev` et suppression du fichier `services/annuaire/chart/templates/service.yaml`.
- **Observation.** Le `Service/annuaire-dev-annuaire` disparait du cluster a `t+15s`. L'application reste `Healthy`, car le `Deployment` et l'`Ingress` existent toujours, meme si l'ingress n'a plus de backend cible operationnel. Le commit de revert `c07b21b` recree ensuite le `Service` et ramene l'app a un etat normal en 16 secondes.
